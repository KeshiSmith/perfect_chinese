...
class addon_updater_install_popup(bpy.types.Operator):
    ...
    bl_description = "检查和显示获取当前更新的弹出框菜单"
    ...
    ignore_enum = bpy.props.EnumProperty(
        ...
        items=[
            ("install","立刻更新","立刻安装更新"),
            ("ignore","忽略", "忽略这次更新以阻止弹窗提醒"),
            ("defer","推迟","推迟选择直到下次 blender 会话")
        ],
        options={'HIDDEN'}
    )
...
class addon_updater_check_now(bpy.types.Operator):
    ...
    bl_description = "立刻为插件 '完美中文' 检查更新".format(
        x=updater.addon)
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def execute(self,context):
        ...
        if not settings:
            if updater.verbose:
                print("无法获取 {} 的偏好设置, 跳过检查更新".format(
                    __package__))
            return {'CANCELLED'}
    ...
    def execute(self,context):
        ...
        if updater.update_ready == True:
            ...
            try:
                ...
                # should return 0, if not something happened
                if updater.verbose:
                    if res==0: print("更新器成功返回")
                    else: print("更新器返回 "+str(res)+", 发生错误")
            except Exception as e:
...
def post_update_callback(module_name, res=None):
    ...
    if res==None:
        ...
        if updater.verbose:
            print("{} 更新器: 运行 POST 更新回调".format(updater.addon))
...
def check_for_update_nonthreaded(self, context):
    ...
    if update_ready == True:
        ...
        if updater.verbose: print("没有更新准备")
...
def update_notice_box_ui(self, context):
    ...
    col.label(text="更新准备完毕!",icon="ERROR")
    col.separator()
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.auto_reload_post_update == False:
        saved_state = updater.json
        if "just_updated" in saved_state and saved_state["just_updated"] == True:
            row.label(text="重启 Blender 以完成更新", icon="ERROR")
            return
    ...
    if updater.error != None and updater.error_msg != None:
        ...
        row.label(text="上次检查时间: " + lastcheck)
    else:
