...
from ..utils import addon_name, addon_name_cn, get_preferences
...
class PC_TabPanel(Panel, PC_Registerable):
    ...
    bl_region_type = 'UI'
    bl_category = addon_name_cn
    bl_label = addon_name_cn
    
    @classmethod
    def pc_register(cls):
        if get_preferences().sidebar_toggle:
            cls.bl_category = get_preferences().tab_category
            register_class(cls)
