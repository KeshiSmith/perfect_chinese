...
class SingletonUpdater:
    ...
    def __str__(self):
        return "更新器, 用户: {a}, 仓库: {b}, 地址: {c}".format(
    ...
    def get_tags(self):
        ...
        if self._tags is None:
            ...
            self.print_verbose("发现 {} 分支, 没有发布版: {}".format(
                branch, self._tags[0]))
    ...
    def unpack_staged_zip(self, clean=False):
        ...
        self.print_verbose(
            "开始从 zip 文件解压源代码:" + str(self._source_zip))
        ...
        for name in zfile.namelist():
            ...
            if name.endswith(zsep):
                try:
                    ...
                    self.print_verbose(
                        "解压 - mkdir: " + os.path.join(outdir, sub_path))
                ...
                with open(os.path.join(outdir, sub_path), "wb") as outfile:
                    ...
                    outfile.write(data)
                    self.print_verbose(
                        "解压 - create: " + os.path.join(outdir, sub_path))
        ...
        if clean:
            try:
                ...
                for f in files:
                    ...
                    self.print_verbose(
                        "清理移除的文件 {}".format(os.path.join(base, f)))
                for f in folders:
            ...
            for file in files:
                ...
                if os.path.isfile(dest_file):
                    ...
                    if replaced:
                        ...
                        self.print_verbose(
                            "重写文件 " + os.path.basename(dest_file))
                    else:
                        self.print_verbose(
                            "格式与 {}, 不匹配, 不进行重写".format(
                                os.path.basename(dest_file)))
                else:
                    # File did not previously exist, simply move it over.
                    os.rename(srcFile, dest_file)
                    self.print_verbose(
                        "新文件 " + os.path.basename(dest_file))
